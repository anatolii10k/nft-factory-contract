...
contract("ERC721Sale", (accounts) => {
    ...
    before(async () => {
        await IkonicERC721Token.new(
            ...
            ikonicerc721token_contract = instance;
        });
        ...
        await IkonicToken.new({ from: accounts[1] }).then(function (instance) {
            ikonictoken_contract = instance;
        });
        ...
        await servicefee_contract.setIkonicTokenContract(ikonictoken_contract.address, { from: accounts[1] })
    ...
    const mint = async (minter, tokenId, fees = []) => {
        ...
        await ikonicerc721token_contract.mint(
            tokenId,
            fees,
            tokenURI,
            { from: minter }
    ...
    describe("buy", () => {
        it("works and costs something (minter:6)", async () => {
            ...
            await ikonicerc721token_contract.setApprovalForAll(
                transferproxy_contract.address,
                true,
                { from: accounts[6] }
            );

            const messageValue = new BN('1020000000000000000');
            const nonce = await salenonceholder_contract.getNonce(
                ikonicerc721token_contract.address,
                tokenId,
                accounts[6]
            );

            await sale_contract.setApproveSaleDate(ikonicerc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-08-31')), { from: accounts[6] })
            let saleInfo = await sale_contract.getSaleInfo(accounts[6], tokenId, {from: accounts[9]});
            assert.equal(saleInfo.approvedSaleDate, getUnixEpochTimeStamp(new Date('2021-08-31')));
            ...
            await sale_contract.buy(
                ikonicerc721token_contract.address, //IERC721 token,
                tokenId, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (IkonicERC721Token)
            ...
            // Owner changed
            const newOwner = await ikonicerc721token_contract.ownerOf(tokenId);
            ...
            console.log("sellerBalanceBeforeSale", sellerBalanceBeforeSale);
            ...
            assert.equal(serviceFeeRecipientGainedBalance.toString(), new BN('40000000000000000').toString());
...
/*
        it("fails when buying for another value (minter:6)", async () => {
            ...
            const saleContractBuySignature = soliditySha3(
                ikonicerc721token_contract.address, // token
                tokenId, // tokenId
                messageValue, // price
                0 // nonce
            );
            ...
            await sale_contract.setApproveSaleDate(ikonicerc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[6] })
        ...
        it("fails without minted token owner approval (minter:3)", async () => {
            ...
            // The owner of the token has to approve the transferproxy for sale == settings something on sale
            // DISABLED TO TEST --> await ikonicerc721token_contract.setApprovalForAll(transferproxy_contract.address, true, {from: accounts[3]});
            ...
            await sale_contract.setApproveSaleDate(ikonicerc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[3] })
        ...
        it("ApproveAll only needed once per minter (minter:4)", async () => {
            ...
            await sale_contract.setApproveSaleDate(ikonicerc721token_contract.address, tokenId1, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[4] })

            // Sold one token
            await sale_contract.buy(
                ikonicerc721token_contract.address, //IERC721 token,
                tokenId1, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (IkonicERC721Token)
            ...
            const newOwner = await ikonicerc721token_contract.ownerOf(tokenId1);
            assert.equal(accounts[7], newOwner);

            await sale_contract.setApproveSaleDate(ikonicerc721token_contract.address, tokenId2, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[4] })

            // Sold two tokens
            await sale_contract.buy(
                ikonicerc721token_contract.address, //IERC721 token,
                tokenId2, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (IkonicERC721Token)
                accounts[4], //address payable owner, the owner of the token from who we are buying the token from
                saleApprovalSignature2.signature, //signtaure
                { from: accounts[8], value: messageValue }
            );

            const newOwner2 = await ikonicerc721token_contract.ownerOf(tokenId2);
        ...
        it("does not work if prev owner try to approve", async () => {
            ...
            // Owner changed
            let newOwner = await ikonicerc721token_contract.ownerOf(tokenId);
            assert.equal(accounts[7], newOwner);
        ...
        it("can't buy token again once it was bought.", async () => {
            ...
            let thrownError;
            try {
                await sale_contract.setApproveSaleDate(ikonicerc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[8] })
            } catch (error) {
                thrownError = error;
            }
        ...
        it("can't buy if the token is not open to sale", async () => {
            ...
            await sale_contract.setApproveSaleDate(ikonicerc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-05-30')), { from: accounts[6] })
        ...
        it("can't set sale date if not owner", async () => {
            ...
            try {
                await sale_contract.setApproveSaleDate(ikonicerc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-05-30')), { from: accounts[5] })
            } catch (error) {
                thrownError = error;
            }

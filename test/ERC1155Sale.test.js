...
contract("ERC1155Sale", (accounts) => {
    var ikonicerc1155token_contract;
    ...
    before(async () => {
        await IkonicERC1155Token.new(
            ...
            ikonicerc1155token_contract = instance;
        });
    ...
    const mint = async (minter, tokenId, supply = 1, fees = []) => {
        ...
        await ikonicerc1155token_contract.mint(
            tokenId,
            fees,
            supply,
            tokenURI,
    ...
    describe("buy", () => {
        it("works and costs something (minter:6)", async () => {
            ...
            await ikonicerc1155token_contract.setApprovalForAll(
                transferproxy_contract.address,
                true,
                { from: accounts[6] }
            );
            ...
            await sale_contract.setApprovedSaleDate(ikonicerc1155token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[6] })
            ...
            await sale_contract.buy(
                ikonicerc1155token_contract.address, //IERC1155 token,
                tokenId, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (IkonicERC1155Token)
            ...
            // Owner changed
            const ownedAmount = await ikonicerc1155token_contract.balanceOf(
                accounts[7],
                tokenId
            );
...
/*
        it("fails when buying for another value (minter:6)", async () => {
            ...
            const nonce = await salenonceholder_contract.getNonce(
                ikonicerc1155token_contract.address,
                tokenId,
                accounts[6]
            );

            const saleContractBuySignature = soliditySha3(
                ikonicerc1155token_contract.address, // token
                tokenId, // tokenId
        ...
        it("fails without minted token owner approval (minter:3)", async () => {
            ...
            const messageValue = new BN('1025000000000000000');
            const nonce = await salenonceholder_contract.getNonce(ikonicerc1155token_contract.address, tokenId, accounts[3]);
            ...
            await sale_contract.setApprovedSaleDate(ikonicerc1155token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[3] })
            ...
            const ownedAmount = await ikonicerc1155token_contract.balanceOf(accounts[7], tokenId);
        ...
        it("does not work if prev owner try to approve", async () => {
            ...
            // Owner changed
            let ownedAmount = await ikonicerc1155token_contract.balanceOf(
                accounts[7],
                tokenId
            );
            assert.equal(1, ownedAmount);
...
/*
    describe("buy with multiple supply", () => {
        ...
        it("can't buy if token is not open to sale", async () => {
            ...
            await sale_contract.setApprovedSaleDate(ikonicerc1155token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2022-03-31')), { from: accounts[6] })
        ...
        it("can't set sale date if owner doesn not have amount of token", async () => {
            ...
            let thrownError;
            try {
                await sale_contract.setApprovedSaleDate(ikonicerc1155token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[5] })
            } catch (error) {
                thrownError = error;
            }
    ...
    describe("fees", () => {
        it("fees are correct sent to recipients & service fee recipient", async () => {
            ...
            await sale_contract.setApprovedSaleDate(ikonicerc1155token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[7] })
            ...
            // Owner changed
            ownedAmount = await ikonicerc1155token_contract.balanceOf(
                accounts[8],
                tokenId
            );
            assert.equal(50, ownedAmount);
        ...
        it("complex buying", async () => {
            ...
            assert.equal(
                await ikonicerc1155token_contract.balanceOf(accounts[7], tokenId),
                1
            );
            assert.equal(
                await ikonicerc1155token_contract.balanceOf(accounts[6], tokenId),
                4
            );
            ...
            assert.equal(
                await ikonicerc1155token_contract.balanceOf(accounts[8], tokenId),
                1
            );
            assert.equal(
                await ikonicerc1155token_contract.balanceOf(accounts[6], tokenId),
